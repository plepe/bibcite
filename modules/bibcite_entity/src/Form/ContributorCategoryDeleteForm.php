 ...
 */
class ContributorCategoryDeleteForm extends EntityConfirmFormBase {
  ...
  public function getQuestion() {
    if (!$this->inUse) {
      ...
      return $this->t('Category %name used in reference(s) and cannot be deleted.', ['%name' => $this->entity->label()]);
    }
  }
  ...
  public function getCancelUrl() {
    return new Url('entity.bibcite_contributor_category.collection');
  }
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    if ($this->inUse) {
      $form_state->setError($form, $this->t('Category %name used in reference(s) and cannot be deleted.', ['%name' => $this->entity->label()]));
    }
